...
# ⬇️ [수정] RetrievalQA 대신 create_retrieval_chain과 create_stuff_documents_chain을 임포트합니다.
# try:
...
        _       job_pref     = st.text_input("선호직무", placeholder="예: 개발자")
...
      <img src="[https://raw.githubusercontent.com/seungcheoll/busan/main/image/logo_guide.png](https://raw.githubusercontent.com/seungcheoll/busan/main/image/logo_guide.png)" 
...
          <hr class="custom"/>
          <img
            src="[https://raw.githubusercontent.com/seungcheoll/busan/main/image/flow.png](https://raw.githubusercontent.com/seungcheoll/busan/main/image/flow.png)"
...
                <img src='[https://raw.githubusercontent.com/seungcheoll/busan/main/image/jobis.png](https://raw.githubusercontent.com/seungcheoll/busan/main/image/jobis.png)' 
...
                # [중요] 템플릿 파일(.txt)들 내부의 {question}을 {input}으로 미리 수정해야 합니다!
                prompt = PromptTemplate.from_template(formatted_template)
                
                # ⬇️ [수정] RetrievalQA.from_chain_type 대신 2단계로 체인을 생성합니다.
                # 1. LLM과 Prompt를 결합하여 '문서 처리 체인' 생성
                document_chain = create_stuff_documents_chain(
                    llm=st.session_state.llm,
                    prompt=prompt
                )
                
                # 2. Retriever와 '문서 처리 체인'을 결합하여 '검색 체인' 생성
...
                )
                ...
                last_error = None # 에러 로깅용
                for _ in range(3):  # 최대 3번만 시도
                    try:
                        # ⬇️ [수정] invoke 인자 키를 'query'에서 'input'으로 변경합니다.
                        result = qa_chain.invoke({"input": query})
                        # ⬆️ [수정]

                        # ⬇️ [수정] 반환 결과 키를 'result'에서 'answer'로 변경합니다.
                        text = result["answer"]
...
                        st.session_state.company_name_by_gpt = text["기업명"]
                        ...
                        # ⬇️ [수정] 반환 결과 키를 'source_documents'에서 'context'로 변경하고, try 블록 안으로 이동합니다.
                        st.session_state.source_docs = result["context"]
                        # ⬆️ [수정]
                        break # 성공 시 루프 탈출
                    except Exception as e: # [수정] 구체적인 에러를 변수에 저장
                        last_error = e
                        continue
                else: # for-else: 3번 모두 실패(break 안됨) 시 실행
                    st.error(f"응답 처리에 실패했습니다: {last_error}")
            
            # ⬇️ [수정] 이 라인은 위 try 블록 안으로 이동했습니다. (성공 시에만 실행되도록)
...
                # 필드 분류
                basic_fields = [
        _           '회사명', '잡코리아 주소','홈페이지', '업 종', '상세업종', '사업분야',
                    '평균초임', '평균연봉', '기업분류',
                    '매출액', '직원수(계)',
                     '도로명', '주요제품 / 서비스'
                ]
...
    _           welfare_fields = [f'복리후생{i}' for i in range(1, 13)]
        
                # 병합 유틸
...
                  _     st.session_state.content_to_gpt.append(content)
                        st.write(content)
            
        # 3️⃣ JOBKOREA
...
        
            with col1:
        _       selected = st.session_state.get('selected_rows', [])
                if selected:
...
    _               st.caption("※ 전체 기업 분포를 표시 중입니다.")
...
            ]
    
        # ⬇️ [수정] st.session_state.source_docs가 'context' 키로 저장되었으므로, 이 키의 존재 여부를 확인합니다.
        # (기존 'source_documents'는 'context'로 변경되었으므로, 키 이름만 다를 뿐 로직은 동일하게 작동합니다.)
...
                    <img src='[https://raw.githubusercontent.com/seungcheoll/busan/main/image/GPT_image2.png](https://raw.githubusercontent.com/seungcheoll/busan/main/image/GPT_image2.png)' 
...
                            <img 
                                src='[https://raw.githubusercontent.com/seungcheoll/busan/main/image/jobis.png](https://raw.githubusercontent.com/seungcheoll/busan/main/image/jobis.png)' 
...
                st.markdown(
                    f"""
                    <div style='padding:12px; border-radius:8px; background-color:#e0f7fa;s                               width:fit-content; margin-left:auto; color:black;'>
                        {msg['content']}
                    </div>
                    """, unsafe_allow_html=True
                )
